...
function ssc_scrollArray(e, t, n, r) {
  ...
  let i = function() {
    ...
    for (let a = 0; a < ssc_que.length; a++) {
      let f = ssc_que[a];
      let l = s - f.start;
      let c = l >= ssc_animtime;
      let h = c ? 1 : l / ssc_animtime;
      if (ssc_pulseAlgorithm) {
        h = ssc_pulse(h)
      }
      let p = f.x * h - f.lastX >> 0;
      let d = f.y * h - f.lastY >> 0;
      o += p;
      u += d;
      f.lastX += p;
      f.lastY += d;
...
//   }
//   let t = e.target;
//   let n = ssc_overflowingAncestor(t);
...
function ssc_overflowingAncestor(e) {
  ...
  do {
    let r = ssc_cache[ssc_uniqueID(e)], overflow;
    if (r) {
...
let ssc_initdone = false;
let ssc_fixedback = true;
let ssc_root = document.documentElement;
